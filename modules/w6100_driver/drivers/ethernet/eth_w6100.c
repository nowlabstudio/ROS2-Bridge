...
static void w6100_rx(const struct device *dev)
{
	...
	pkt = net_pkt_rx_alloc_with_buffer(ctx->iface, rx_len,
			AF_UNSPEC, 0, K_MSEC(CONFIG_ETH_W6100_TIMEOUT));
	if (!pkt) {
...
#define W6100_MAC_ADDR_INIT(inst) \
	COND_CODE_1(DT_NODE_HAS_PROP(DT_DRV_INST(inst), local_mac_address), \
