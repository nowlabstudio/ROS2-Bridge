...
class RCTeleopNode(Node):
    ...
    def __init__(self):
        ...
        self.declare_parameter('max_wheel_speed', 4.5)
        self.declare_parameter('wheel_separation', 0.4)
        self.declare_parameter('max_linear_speed', 4.5)
        ...
        if self.mixing_mode == 'tank':
            self.get_logger().info(
                "RC Teleop [tank] started: left=%s right=%s -> %s | "
                "max_wheel=%.1f m/s wheel_sep=%.2f m | mode_switch: %s | rate: %.0f Hz"
                % (ch_a_label, ch_b_label, cmd_vel_topic,
                   self.max_wheel_speed, self.wheel_separation,
                   mode_switch_topic, publish_rate))
        else:
            self.get_logger().info(
                "RC Teleop [arcade] started: %s + %s -> %s | "
                "max_lin=%.1f max_ang=%.1f | mode_switch: %s | rate: %.0f Hz"
                % (ch_a_label, ch_b_label, cmd_vel_topic,
    ...
    def _publish_twist(self):
        ...
        if not self.estop_active:
            if self.mixing_mode == 'tank':
                v_left = self.ch_a * self.max_wheel_speed
                v_right = self.ch_b * self.max_wheel_speed
                msg.twist.linear.x = (v_left + v_right) / 2.0
                msg.twist.angular.z = (v_right - v_left) / self.wheel_separation
            else:
